...
def compute_score(
    text: str, patterns_themes_df: pd.DataFrame
    ...
    for _, (pattern, themes) in patterns_themes_df.iterrows():
        ...
        pattern_matches_dict = {
            k: v for k, v in pattern_matches_dict.items() if len(v) > 0
        }

    return score, list(matched_themes), pattern_matches_dict


def score_school_courses(
    school: str, year: int, output_dir: str, dictionary_version: str = "1.1"
) -> None:
    """
    ...
    :param dictionary_version: version of the dictionary that should be used to score the courses without the extnsion '.json'.

    :return: None
    """

    # Loading crawling results
    courses_fn = CRAWLING_OUTPUT_FOLDER.joinpath(f"{school}_courses_{year}.json")
    courses_df = pd.read_json(open(courses_fn, "r"), dtype={"id": str})

    # Load fields on which the scoring has to be done
    scoring_fields_fn = Path(__file__).parent.joinpath(f"scoring_fields.csv")
    fields = ["name"] + pd.read_csv(scoring_fields_fn, index_col=0).loc[
        school, "fields"
    ].split(";")
    for field in fields:
        assert (
            field in courses_df.columns
        ), f"Error: the courses DataFrame doesn't contain a column {field}"
        # Convert Nans to "
    ...
    for lang in ACCEPTED_LANGUAGES:
        themes_fn = Path(__file__).parent.joinpath(
            f"dictionnary_{dictionary_version}/{lang}.csv"
        )
    ...
    for lang in ACCEPTED_LANGUAGES:
        themes_fn = Path(__file__).parent.joinpath(f"dedicated/{lang}.csv")
        dedicated_patterns_dict[lang] = pd.read_csv(
            themes_fn, converters={"themes": literal_eval}
        )
    ...
    scores_df = pd.DataFrame(
        0, index=courses_df.index, columns=themes + ["dedicated"], dtype=int
    )
    for i, (idx, name, scoring_text, full_text) in courses_df.reset_index()[
        ["id", "name", "scoring_text", "full_text"]
    ].iterrows():
        ...
        if len(languages) == 0:
            languages = list(
                set(courses_df.loc[idx, "languages"]).intersection(
                    set(ACCEPTED_LANGUAGES)
                )
            )
            if len(languages) == 0:
                continue
        ...
        for language in languages:
            score, matched_themes, shift_patterns_matches_dict = compute_score(
                scoring_text, patterns_dict[language]
            )
            ...
            if score == 1:
                ...
                patterns_matches_dict[f"{idx}: {name}"][
                    language
                ] = shift_patterns_matches_dict
                # Check if course is dedicated
                if any(
                    [
                        re.search(p, clean_text(name)) is not None
...
if __name__ == "__main__":
    ...
    arguments["output_dir"] = SCORING_OUTPUT_FOLDER
    arguments["dictionary_version"] = "1.1"
